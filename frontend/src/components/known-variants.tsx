...
export default function KnownVariants({
  ...
  const pdf = new jsPDF({ unit: "pt", format: "a4" }); // points for precise layout
  const marginX = 40;
  let y = 40;
  ...
  const rows = clinvarVariants.map((v) => [
    ...
    v.location || "-",
    v.evo2Result?.prediction || "-", // if analyzed, show Evo2 prediction
  ]);

  // 4) Render table (auto-paginates if long)
  autoTable(pdf, {
    head: [columns],
    body: rows,
    startY: y + 20,
    ...
    headStyles: {
      fillColor: [233, 238, 234], // light green-ish to match your UI vibe
      textColor: [60, 79, 61],
      fontStyle: "bold",
    },
    bodyStyles: {
    ...
    // Prevent super-wide columns; autoTable will wrap text
    columnStyles: {
      0: { cellWidth: 180 }, // Variant
      1: { cellWidth: 120 }, // Type
      2: { cellWidth: 160 }, // Clinical Significance
      3: { cellWidth: 100 }, // Location
      4: { cellWidth: 100 }, // Evo2
    },
    didDrawPage: (data) => {
  ...
  // 5) Save
  const fileDate = generatedOn.toISOString().slice(0, 10); // YYYY-MM-DD
  const filename = `known_variants_${geneLabel}_${fileDate}.pdf`.replace(/\s+/g, "_");
  pdf.save(filename);
};
